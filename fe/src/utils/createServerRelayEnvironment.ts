...
const HTTP_ENDPOINT = process.env.API_ENDPOINT_GRAPHQL;
const IS_SERVER = typeof window === typeof undefined;
const CACHE_TTL = 5 * 1000; // 5 seconds, to resolve preloaded results

export async function networkFetch<T = GraphQLResponse>(request: RequestParameters, variables: Variables): Promise<T> {
    if (!HTTP_ENDPOINT) {
        throw new Error('No GraphQL endpoint defined!');
    }

    const { data: json } = await axios(HTTP_ENDPOINT, {
        data: JSON.stringify({ query: request.text, variables }),
        headers: {
